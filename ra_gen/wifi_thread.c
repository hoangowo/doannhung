...
static StaticTask_t wifi_thread_memory;
                ...
                static uint8_t wifi_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t wifi_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.wifi_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
sci_uart_instance_ctrl_t g_uart_esp_ctrl;

baud_setting_t g_uart_esp_baud_setting = {
...
const sci_uart_extended_cfg_t g_uart_esp_cfg_extend = { .clock =
		...
		.noise_cancel = SCI_UART_NOISE_CANCELLATION_DISABLE, .rx_fifo_trigger =
				...
				&g_uart_esp_baud_setting, .flow_control =
...
#undef RA_NOT_DEFINED
		.rxi_ipl = (4), .txi_ipl = (4), .tei_ipl = (4), .eri_ipl = (4),
...
const uart_instance_t g_uart_esp = { .p_ctrl = &g_uart_esp_ctrl, .p_cfg =
		&g_uart_esp_cfg, .p_api = &g_uart_on_sci };
...
void wifi_thread_create(void) {
	...
	wifi_thread = xTaskCreateStatic(
                    ...
                    #endif
			wifi_thread_func, (const char*) "WiFi Thread", 1024 / 4, // In words, not bytes
